...
| `service-scaffolding-reviewer` | **sonnet** | Golden-path scaffold completeness gate — mechanically checks every paved-road component exists (CI, observability, API contract stub, tests, runbook, resource limits set not unbounded, catalog entry with owner, scorecard). Deterministic file-presence checks. Invoked by `service-scaffolding` skill. |
