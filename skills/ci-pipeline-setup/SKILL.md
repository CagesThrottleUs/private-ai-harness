...
**Skip if:** a comprehensive, reviewed CI pipeline already exists and passes `ci-reviewer` agent with no Critical findings.
...
pipeline {
    ...
    stages {
        ...
        stage('Unit Tests') {
            steps {
                // test command with coverage threshold enforcement
            }
            post {
                always {
        ...
        stage('Build') {
            when { branch 'main' }
            steps {
                // build command — produces immutable artifact
            }
        }
        ...
        stage('Deploy Staging') {
            ...
            steps {
                // deploy command
                sh 'curl -sf $STAGING_URL/health || exit 1'
            }
        }
    }
...
orbs:
  # add language-specific orbs here (e.g. node, python, go)
  ...
  ci:
    jobs:
      ...
      - security-scan:
          requires: [lint]
      - build:
          requires: [unit-tests, security-scan]
          filters:
            branches:
              only: main
      - deploy-staging:
...
jobs:
  ...
  build:
    ...
    steps:
      ...
      - persist_to_workspace:
          root: .
          paths: [dist/]   # adjust for your build output
  ...
  deploy-staging:
    ...
    steps:
      - attach_workspace:
          at: .
      # [deploy command using workspace artifact]
```
