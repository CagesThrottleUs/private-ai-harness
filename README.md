...
(`sessionStart`/`agentStop`/`preToolUse`/`postToolUse`/...; no `preCompact`
event exists) configured via personal hook files in `~/.copilot/hooks/*.json`
— NOT `config.json`, which silently ignores an unrecognized `hooks` key.
`scripts/install-copilot.sh` writes `~/.copilot/hooks/private-ai-harness.json`,
wiring `scripts/copilot-notify.sh` into `SessionStart`/`Stop`/`PreToolUse`/
`PostToolUse` there — without the tool-use pair, the only audible cue all
session is the single Stop beep at the end.
...
5. **Skills** — `impeccable`, `emilkowalski/skill`, `taste-skill` (`mukul975/Anthropic-Cybersecurity-Skills` is commented out — uncomment in `install-claude.sh` to enable)
